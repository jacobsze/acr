...
def volunteers():
    ...
    return render_template("admin_volunteers.html", volunteers=all_users)
...
def add_volunteer():
    ...
    db.session.commit()
    flash(f"{name} has been added as a volunteer.", "success")
    return redirect(url_for("admin.volunteers"))
...
def deactivate_volunteer(user_id):
    ...
    user.active = False
    user.session_token = None  # Invalidate active session
    db.session.commit()
...
def save_regular_schedule():
    ...
    # Parse checkbox names: shift_{user_id}_{dow}_{AM|PM}
